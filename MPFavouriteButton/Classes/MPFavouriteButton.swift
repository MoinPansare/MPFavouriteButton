//
//  MPFavouriteButton.swift
//  Pods
//
//  Created by Moin on 24/01/17.
//
//

import UIKit

public class MPFavouriteButton: UIButton {
...
//        self.backgroundColor = UIColor.init(patternImage: image)
    ...
    private func createLayers(image image: UIImage!) {
        ...
        let imageFrame = CGRect(x: 0, y: 0, width: frame.size.width / 2, height: frame.size.height / 2)
        
        let imgCenterPoint = CGPoint(x: self.frame.size.width / 2, y: self.frame.size.height / 2)
        ...
        imageShape.frame = CGRect(x: 0, y: 0, width: self.frame.size.width, height: self.frame.size.height);
        imageShape.path = UIBezierPath(rect: imageShape.frame).cgPath
        imageShape.fillColor = imageColorOff.cgColor
    ...
    private func addTargets() {
        ...
        // add target
        //===============
        self.addTarget(self, action: #selector(MPFavouriteButton.touchDown(sender:)), for: .touchDown)
        self.addTarget(self, action: #selector(MPFavouriteButton.touchUpInside(sender:)), for: .touchUpInside)
        self.addTarget(self, action: #selector(MPFavouriteButton.touchDragExit(sender:)), for: .touchDragExit)
        self.addTarget(self, action: #selector(MPFavouriteButton.touchDragEnter(sender:)), for: .touchDragEnter)
        self.addTarget(self, action: #selector(MPFavouriteButton.touchCancel(sender:)), for: .touchCancel)

    }
    
    func touchDown(sender: MPFavouriteButton) {
        self.layer.opacity = 0.4
    }
    func touchUpInside(sender: MPFavouriteButton) {
        self.layer.opacity = 1.0
    }
    func touchDragExit(sender: MPFavouriteButton) {
        self.layer.opacity = 1.0
    }
    func touchDragEnter(sender: MPFavouriteButton) {
        self.layer.opacity = 0.4
    }
    func touchCancel(sender: MPFavouriteButton) {
        self.layer.opacity = 1.0
    }
